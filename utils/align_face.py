...
def align_img_mask(img, mask, src_lmks, size=224):
    M = align_with_five_points(src_lmks, size)
    aligned_img = cv2.warpAffine(img, M, (size, size), flags=cv2.INTER_LINEAR)
    aligned_mask = cv2.warpAffine(mask, M, (size, size), flags=cv2.INTER_LINEAR)
    return aligned_img, aligned_mask, back_matrix(M[:2])
